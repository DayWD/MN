...
double secant(double (*f)(double), double a, double b, int M, double eps, double delta, double fa, double fb)
{
    double s;
    ...
    for(;M!=0;)
    {
        ...
        fa = f(a);
        M--;
        if(fabs(fa) < eps or fabs(b - a) < delta)
            return a;
    }
    return a;
}
...
)
{
    ...
    if(signbit(fa) == signbit(fb))  //  Czesc algo bisekcji. Na podstawie tresci z bacy wrzucam to tutaj, aby nie było zamieszania
    {
        return  secant(f,a,b,M,eps,delta,fa,fb);
    }
    ...
    return secant(f,a,b,M,eps,delta,u,v);
};
